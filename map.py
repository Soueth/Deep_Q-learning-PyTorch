...
class MyPaintWidget(Widget):
    def __init__(self, **kwargs):
        ...
        self.lines = []  # Lista para armazenar as linhas criadas
    
    def load_map(self, lines_data):
        with self.canvas:
            Color(0.8,0.7,0)
            for line_data in lines_data:
                line = Line(points=line_data['points'], width=line_data['width'])
    ...
    def on_touch_down(self, touch): # adiciona areia quando clicamos com o botão esquerdo
        ...
        with self.canvas:
            Color(0.8,0.7,0)
            d = 10.
            line = Line(points = (touch.x, touch.y), width = 10)
            touch.ud['line'] = line
            self.lines.append(line)
            last_x = int(touch.x)
            last_y = int(touch.y)
...
class CarApp(App):
    ...
    def build(self): # building the app
        ...
        savemapbtn = Button(text = 'save map', pos = (3 * parent.width, 0))
        loadmapbtn = Button(text = 'load map', pos = (4 * parent.width, 0))
        clearbtn.bind(on_release = self.clear_canvas)
        savebtn.bind(on_release = self.save)
        loadbtn.bind(on_release = self.load)
        savemapbtn.bind(on_release = self.save_map)
        loadmapbtn.bind(on_release = self.open_map)
    ...
    def save(self, obj): # save button
        ...
        print("saving brain...")
        brain.save()
        # np.save('mapa.npy', {'sand': sand, 'lines_data': lines_data})
        # self.painter.export_to_png()
        # image.save('/tmp/mapa.png')
        plt.plot(scores)
        plt.show()
    ...
    def load(self, obj): # load button
        ...
        brain.load()
        # sand = np.load('mapa.npy')
                # self.painter.canvas.add(line)
                
        # self.painter  = CoreImage('/tmp/mapa.png')
        # self.painter.add_widget(OneLineAvatarIconListItem)
        # self.painter = mapa['map']
        # sand = mapa['sand']
    ...
    def save_map(self, obj):
        ...
        lines_data = [
            {'points': line.points, 'width': line.width} 
            for line in self.painter.lines
        ]
        
        with open('mapa.pkl', 'wb') as f:
            pkl.dump({'sand': sand, 'lines_data': lines_data}, f)
            
    def open_map(self, obj):
        global sand
        with open('mapa.pkl', 'rb') as f:
            mapa = pkl.load(f)
            sand = mapa['sand']
            lines_data = mapa['lines_data']
            
            self.painter.load_map(lines_data)
